...
void f_analytic_rust(int Q, const double mu, const double lambda,
                     ...
                     double *f1) {
  compute_f_analytic(Q, mu, lambda, dXdx_init, dudX,
                     NUM_COMPONENTS_STORED_ANALYTIC_RUST, *stored_values, f1);
}

void df_analytic_rust(int Q, const double mu, const double lambda,
                      double *ddudX, double **stored_values, double *df) {
  compute_df_analytic(Q, mu, lambda, ddudX, NUM_COMPONENTS_STORED_ANALYTIC_RUST,
                      *stored_values, df);
}
