...
static inline void MatDetAM1Symmetric_t_b(const double A_sym[6],
  ...
  for (ii1 = 0; ii1 < 6; ++ii1)
    A_symb[ii1] = 0.0;
  A_symb[0] = A_symb[0] + (A_sym[1] * A_sym[2] - A_sym[3] * A_sym[3] +
                           A_sym[1] + A_sym[2] + 1.0) *
                              (*detb);
  tempb = A_sym[0] * (*detb);
  A_symb[5] =
      A_symb[5] +
      (A_sym[3] * A_sym[4] - A_sym[5] * A_sym[2] - 2 * A_sym[5]) * (*detb) +
      A_sym[3] * tempb1 - A_sym[2] * tempb0;
  A_symb[4] =
      A_symb[4] +
      (A_sym[5] * A_sym[3] - A_sym[4] * A_sym[1] - 2 * A_sym[4]) * (*detb) +
      A_sym[3] * tempb0 - A_sym[1] * tempb1;
  A_symb[1] = A_symb[1] + (A_sym[0] + A_sym[2] + 1.0) * (*detb) +
              A_sym[2] * tempb - A_sym[4] * tempb1;
  A_symb[2] = A_symb[2] + (A_sym[0] + A_sym[1] + 1.0) * (*detb) +
              A_sym[1] * tempb - A_sym[5] * tempb0;
  A_symb[3] = A_symb[3] + A_sym[5] * tempb1 - 2 * A_sym[3] * (*detb) +
...
compute_grad_psi_tapenade(const double e_sym[6], double e_symb[6],
  ...
  *energyb = 0.0;
  ...
  detbm1b = (fabs(detbm1 + 1) < TOL
                 ? logJb / ((detbm1 + 1) * 2.)
                 : logJb / ((detbm1 + 1) * 2.) + Jb / (2.0 * sqrt(detbm1 + 1)));
...
static inline void MatDetAM1Symmetric_t_b_d(const double A_sym[6],
  ...
  A_symbd[0] =
      (*detb) * ((A_sym[2] + 1.0) * A_symd[1] + (A_sym[1] + 1.0) * A_symd[2] -
                 2 * A_sym[3] * A_symd[3]) +
      temp * (*detbd);
  ...
  A_symbd[5] = A_symbd[5] +
               (*detb) * (A_sym[4] * A_symd[3] + A_sym[3] * A_symd[4] -
                          (A_sym[2] + 2) * A_symd[5] - A_sym[5] * A_symd[2]) +
               temp * (*detbd) + tempb1 * A_symd[3] + A_sym[3] * tempb1d -
               tempb0 * A_symd[2] - A_sym[2] * tempb0d;
  A_symb[5] =
      A_symb[5] + temp * (*detb) + A_sym[3] * tempb1 - A_sym[2] * tempb0;
  temp = A_sym[5] * A_sym[3] - A_sym[4] * A_sym[1] - 2 * A_sym[4];
  A_symbd[4] = A_symbd[4] +
               (*detb) * (A_sym[3] * A_symd[5] + A_sym[5] * A_symd[3] -
                          (A_sym[1] + 2) * A_symd[4] - A_sym[4] * A_symd[1]) +
               temp * (*detbd) + tempb0 * A_symd[3] + A_sym[3] * tempb0d -
               tempb1 * A_symd[1] - A_sym[1] * tempb1d;
  A_symb[4] =
      A_symb[4] + temp * (*detb) + A_sym[3] * tempb0 - A_sym[1] * tempb1;
  A_symbd[1] = A_symbd[1] + (*detb) * (A_symd[0] + A_symd[2]) +
               (A_sym[0] + A_sym[2] + 1.0) * (*detbd) + tempb * A_symd[2] +
               A_sym[2] * tempbd - tempb1 * A_symd[4] - A_sym[4] * tempb1d;
  A_symb[1] = A_symb[1] + (A_sym[0] + A_sym[2] + 1.0) * (*detb) +
              A_sym[2] * tempb - A_sym[4] * tempb1;
  A_symbd[2] = A_symbd[2] + (*detb) * (A_symd[0] + A_symd[1]) +
               (A_sym[0] + A_sym[1] + 1.0) * (*detbd) + tempb * A_symd[1] +
               A_sym[1] * tempbd - tempb0 * A_symd[5] - A_sym[5] * tempb0d;
  A_symb[2] = A_symb[2] + (A_sym[0] + A_sym[1] + 1.0) * (*detb) +
              A_sym[1] * tempb - A_sym[5] * tempb0;
  A_symbd[3] = A_symbd[3] + tempb1 * A_symd[5] + A_sym[5] * tempb1d -
               (2 * (*detb) + 2 * tempb) * A_symd[3] - 2 * A_sym[3] * (*detbd) +
               tempb0 * A_symd[4] + A_sym[4] * tempb0d - 2 * A_sym[3] * tempbd;
  A_symb[3] = A_symb[3] + A_sym[5] * tempb1 - 2 * A_sym[3] * (*detb) +
              A_sym[4] * tempb0 - 2 * A_sym[3] * tempb;
}
...
static inline void MatDetAM1Symmetric_t_nodiff_d(const double A_sym[6],
  ...
  *detd = (temp + A_sym[1] + A_sym[2] + 1.0) * A_symd[0] +
          A_sym[0] * (A_sym[2] * A_symd[1] + A_sym[1] * A_symd[2] -
                      2 * A_sym[3] * A_symd[3]) +
          (temp0 - 2 * A_sym[5]) * A_symd[5] +
          A_sym[5] * (A_sym[4] * A_symd[3] + A_sym[3] * A_symd[4] -
                      A_sym[2] * A_symd[5] - A_sym[5] * A_symd[2]) +
          (temp1 - 2 * A_sym[4]) * A_symd[4] +
          A_sym[4] * (A_sym[3] * A_symd[5] + A_sym[5] * A_symd[3] -
                      A_sym[1] * A_symd[4] - A_sym[4] * A_symd[1]) +
          (A_sym[0] + A_sym[2] + 1.0) * A_symd[1] +
          (A_sym[0] + A_sym[1] + 1.0) * A_symd[2] - 2 * A_sym[3] * A_symd[3];
  *det = A_sym[0] * temp + A_sym[5] * temp0 + A_sym[4] * temp1 + A_sym[0] +
         A_sym[1] + A_sym[2] + A_sym[0] * A_sym[1] + A_sym[0] * A_sym[2] +
         A_sym[1] * A_sym[2] - A_sym[5] * A_sym[5] - A_sym[4] * A_sym[4] -
         A_sym[3] * A_sym[3];
}
...
static inline void compute_grad_psi_d(const double e_sym[6],
  ...
  *energyb = 0.0;
  ...
  detbm1bd =
      (fabs0 < TOL ? (logJbd - temp * 2. * detbm1d) / (2. * (detbm1 + 1))
                   : (logJbd - temp0 * 2. * detbm1d) / (2. * (detbm1 + 1)) +
                         (Jbd - temp1 * 2.0 * result1d) / (2.0 * result1));
...
compute_dtau_sym_fwd_tapenade(const double e_sym[6], const double e_symd[6],
                              ...
                              const double mu, const double mud,
                              double tau_sym[6], double tau_symd[6]) {
  double lambdab = 0.;
  double mub = 0.;
  double energy;
  double energyb = 1.;
